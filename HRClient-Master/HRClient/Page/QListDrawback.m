...
#import "QListDrawback.h"
#import "QViewController.h"
#import "QHttpMessageManager.h"
#import "QMyListDetailModel.h"

@interface QListDrawback (){
...
- (void)pageEvent:(QPageEventType)eventType
{
    ...
    {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(lookDrawBackInfro:) name:kGetMyListDetail object:nil];
...
- (UIView *)viewWithFrame:(CGRect)frame{
    if ([super viewWithFrame:frame]) {
        ...
        scrollView.contentSize = CGSizeMake(frame.size.width, 660);
...
//        第二块
        ...
        UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(contentBeforeW, couponView.frame.size.height - 30, couponView.frame.size.width - 2*contentBeforeW, 1)];
        lineLabel.backgroundColor = [QTools colorWithRGB:223 :223 :223];
        [couponView addSubview:lineLabel];
        
        UILabel *detailTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(2 * beforeW, lineLabel.frame.origin.y + contentBlank, 60, 20)];
        detailTitleLabel.text = @"已退款";
        detailTitleLabel.font = [UIFont systemFontOfSize:13];
        detailTitleLabel.textColor = [QTools colorWithRGB:220 :92 :55];
        [couponView addSubview:detailTitleLabel];
        
        UIButton *examineBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        examineBtn.frame = CGRectMake(couponView.frame.size.width - 2 *beforeW - detailTitleLabel.frame.size.width, lineLabel.frame.origin.y + contentBlank, 60, 20);
        [examineBtn setTitle:@"查看详情" forState:UIControlStateNormal];
        [examineBtn setTitleColor:[QTools colorWithRGB:181 :0 :0] forState:UIControlStateNormal];
        examineBtn.titleLabel.font = [UIFont systemFontOfSize:13];
        [couponView addSubview:examineBtn];
        
        UIButton *checkBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        checkBtn.frame = CGRectMake(contentBeforeW , couponView.deFrameBottom + blank,frame.size.width - 2*contentBeforeW, 30);
...
- (void)lookDrawBackInfro:(NSNotification *)noti{
    listModel = noti.object;
    titleLabel.text = listModel.subject;
    
    NSString *price = [listModel.total stringValue];
    ...
    if ([listModel.returnNotSpending intValue])
    {
    ...
    if ([listModel.returnOverdue intValue])
    {
    ...
    if ([listModel.returnDissatisfied intValue])
    {
    ...
    NSString *str= [listModel.expireDate stringValue] ;//时间戳
    ...
    NSArray *detailArr = [[NSArray alloc] initWithObjects:listModel.payNo,currentDateStr,listModel.verificationCode, nil];
    for (int i = 0; i < 3; i ++) {
    ...
    NSString *str1= [listModel.gmtCreate stringValue] ;//时间戳
    ...
    NSArray *detailArr1 = [[NSArray alloc] initWithObjects:listModel.orderListNo,listModel.phone,currentDateStr1,[listModel.quantity stringValue],listModel.total.stringValue, nil];
    for (int i = 0; i < 5; i ++) {
